...
def main():
    ...
    for i in range(max_epochs):
        ...
        logging.info("[Epoch {:3}]   Loss:  {:8.4}     Train Acc:  {:8.4}%      Val Acc:  {:8.4}%".format(i,
                                                                                                          loss_running / len(
                                                                                                              X_train),
                                                                                                          acc_running / len(
                                                                                                              X_train) * 100,
    ...
    testEx = net.forward(X_test)
    testtest = np.argmax(testEx, axis=1)[:, np.newaxis]
    test_out = np.concatenate((np.expand_dims(np.array(range(4220), dtype=int), axis=1), testtest), axis=1)
    #print(testEx)
    header = np.array([["id", "digit"]])
    test_out = np.concatenate((header, test_out))
    np.savetxt('test_predicted_hw3.csv', test_out, fmt='%s', delimiter=',')
